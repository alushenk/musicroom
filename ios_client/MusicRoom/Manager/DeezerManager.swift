...
class DeezerManager: NSObject {
    ...
    func startDeezer() {
        deezerConnect = DeezerConnect(appId: DeezerConstant.AppKey.appId, andDelegate: self)
        DZRRequestManager.default().dzrConnect = deezerConnect
        self.retrieveTokenAndExpirationDate()
    }

    func login() {
        deezerConnect?.authorize([DeezerConnectPermissionBasicAccess, DeezerConnectPermissionOfflineAccess])
    }
